...
local constants = require "kong.plugins.custom-statsd.constants"
local statsd_logger = require "kong.plugins.custom-statsd.statsd_logger"
...
local function get_api_endpoint(message)
  local uri = re_gsub(message.request.uri,[[[\d\/]+$|(\/)?(?=\?|\#$).*]],"")
  return uri
end

local function get_username_from_jwt()
  local authorization_header = kong.request.get_header("authorization")
    if authorization_header then
        ...
        if m and #m > 0 then
            return jwt_decoder:new(m[1]).claims["preferred_username"]
        end
    end
end
...
local metrics = {
  ...
  status_count_per_user = function (scope_name, message, metric_config, logger, conf, consumer_id)
    local get_consumer_id = get_consumer_id[metric_config.consumer_identifier or conf.consumer_identifier_default]
  ...
  status_count_per_user_per_route = function (_, message, metric_config, logger, conf, consumer_id)
    local get_consumer_id = get_consumer_id[metric_config.consumer_identifier or conf.consumer_identifier_default]
    ...
    if route.id then
      logger:send_statsd(string_format("route.%s.user.%s.status.%s", route.name,
        consumer_id, message.response.status),
        1, logger.stat_types.counter,
        metric_config.sample_rate)
    end
  end,
  request_count_per_api = function (scope_name, message, metric_config, logger, conf)
    local api_endpoint = get_api_endpoint(message)
    if api_endpoint and message.response.status == 200 then
      local stat = string_format("%s.api.%s.count", scope_name,api_endpoint)
      logger:send_statsd(stat, 1, logger.stat_types.counter,
        metric_config.sample_rate)
    end
  end,
  request_count_per_user_per_api = function (scope_name, message, metric_config, logger, conf, consumer_id)
    local get_consumer_id = get_consumer_id[metric_config.consumer_identifier or conf.consumer_identifier_default]
    if not consumer_id then
      return
    end
    -- local request_info = get_request_info(message)

    local api_endpoint = get_api_endpoint(message)
    if api_endpoint  and message.response.status == 200 then
      local stat = string_format("%s.user.%s.api.%s.count", scope_name,consumer_id,api_endpoint)
      logger:send_statsd(stat, 1, logger.stat_types.counter,
        metric_config.sample_rate)
    end
  end,
...
local function log(conf, messages, consumer_id)
  ...
  for _, message in ipairs(messages) do
    ...
    for _, metric_config in pairs(conf.metrics) do
      ...
      if metric then
        metric(name, message, metric_config, logger, conf, consumer_id)
...
function _M.execute(conf)
  ...
  local consumer_id = get_username_from_jwt()
  if not q then
    local batch_max_size = conf.queue_size or 1
    local process = function (entries)
      return log(conf, entries, consumer_id)
    end
